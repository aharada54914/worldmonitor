...
async function dispatch(requestUrl, req, routes, context) {
  ...
  // do the same.
  //
  // Exception: /api/local-instance-config returns only non-sensitive UI
  // display defaults (theme, font, map provider) sourced from WM_INSTANCE_*
  // env vars, and the web/desktop client must read them before it holds the
  // per-session token — so it is intentionally served ahead of the gate.

...
Use `/api/health` as the monitor URL. The endpoint returns HTTP 200 for
application reachability and reports data-pipeline state in the JSON body:

- `availabilityStatus: "UP"` = app and Redis health evaluation are reachable
- `availabilityStatus: "DOWN"` = Redis health evaluation is unavailable
- `dataStatus` = data freshness/coverage state (`HEALTHY`, `WARNING`, `DEGRADED`, `UNHEALTHY`)

For paging monitors, alert only when `"shouldPage":true`. Data freshness
problems remain visible in `problems` and `summary`, but they do not page by
default.
...
```bash
# Alert only when the endpoint explicitly says to page
STATUS=$(curl -s "https://api.worldmonitor.app/api/health?compact=1")
SHOULD_PAGE=$(echo "$STATUS" | jq -r '.shouldPage')
if [ "$SHOULD_PAGE" = "true" ]; then
  echo "PAGE: health monitor requires attention"
...
| **HTTP 503** | No; inspect `shouldPage` and `availabilityStatus` | Redis/auth failures only |

...
if [ -f "$OVERRIDE" ]; then
  ...
  grep -E '^\s+[A-Z_]+:' "$OVERRIDE" \
    ...
    | sed 's/: */=/' \
    | sed "s/[\"']//g" \
    | grep -E '^(NASA_FIRMS|GROQ|AISSTREAM|FRED|FINNHUB|EIA|ACLED_ACCESS_TOKEN|ACLED_EMAIL|ACLED_PASSWORD|CLOUDFLARE|AVIATIONSTACK|OPENROUTER_API_KEY|LLM_API_URL|LLM_API_KEY|LLM_MODEL|OLLAMA_API_URL|OLLAMA_MODEL|WTO_API_KEY|ICAO_API_KEY|UCDP_ACCESS_TOKEN|WINGBITS_API_KEY|CONSUMER_PRICES_CORE_BASE_URL)' \

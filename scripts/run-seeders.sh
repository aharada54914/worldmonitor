...
# Local API base for seeders that warm RPC caches (seed-insights, etc.).
# Token/URL for the Redis REST proxy are resolved securely below (#3804).
WM_API_BASE_URL="${WM_API_BASE_URL:-http://localhost:3000}"
API_BASE_URL="${API_BASE_URL:-$WM_API_BASE_URL}"
export WM_API_BASE_URL API_BASE_URL
...
if [ -f "$OVERRIDE" ]; then
  ...
  grep -E '^\s+[A-Z_]+:' "$OVERRIDE" \
    ...
    | sed 's/: */=/' \
    | sed "s/[\"']//g" \
    | grep -E '^(NASA_FIRMS|GROQ|AISSTREAM|FRED|FINNHUB|EIA|ACLED_ACCESS_TOKEN|ACLED_EMAIL|ACLED_PASSWORD|CLOUDFLARE|AVIATIONSTACK|OPENAQ_API_KEY|WAQI_API_KEY|OPENROUTER_API_KEY|LLM_API_URL|LLM_API_KEY|LLM_MODEL|OLLAMA_API_URL|OLLAMA_MODEL|WTO_API_KEY|ICAO_API_KEY|UCDP_ACCESS_TOKEN|WINGBITS_API_KEY|CONSUMER_PRICES_CORE_BASE_URL)' \
